...
parseFor (KL.Word "for":xs) = let (assign_expr, rest) = parseExpression xs in parseFor' rest assign_expr
parseFor (x:xs)             = throw $ createParsingError "parseFor" [KL.Word "for"] (Just x) xs

parseFor' :: [KL.Token]  -> Expression -> (For, [KL.Token])
parseFor' []            _           = throw $ createParsingError "parseFor" [KL.Comma] Nothing []
parseFor' (KL.Comma:xs) assign_expr = let (cmp_expr, rest) = parseExpression xs in parseFor'' rest assign_expr cmp_expr
parseFor' (x:xs)        _           = throw $ createParsingError "parseFor" [KL.Comma] (Just x) xs

parseFor'' :: [KL.Token] -> Expression -> Expression -> (For, [KL.Token])
parseFor'' []             _          _        = throw $ createParsingError "parseFor" [KL.Comma] Nothing []
parseFor'' (KL.Comma:xs) assign_expr cmp_expr = let (inc_expr, rest) = parseExpression xs in parseFor''' rest assign_expr cmp_expr inc_expr
parseFor'' (x:xs)         _          _        = throw $ createParsingError "parseFor" [KL.Comma] (Just x) xs

parseFor''' :: [KL.Token] -> Expression -> Expression -> Expression -> (For, [KL.Token])
parseFor''' []                _           _        _        = throw $ createParsingError "parseFor" [KL.Word "in"] Nothing []
parseFor''' (KL.Word "in":xs) assign_expr cmp_expr inc_expr = let (core_expr, rest) = parseExpressions xs in
    (For assign_expr cmp_expr inc_expr core_expr, rest)
parseFor''' (x:xs)            _           _        _        = throw $ createParsingError "parseFor" [KL.Word "in"] (Just x) xs
